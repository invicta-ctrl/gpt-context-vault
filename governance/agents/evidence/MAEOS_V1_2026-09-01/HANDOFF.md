...
POST-MERGE PORTABILITY REPAIR: added the narrow `/vendor/maeos/curated/** text eol=lf` checkout rule and verifier regression requiring `git check-attr text=set eol=lf` for all nine registered curated sources. Under `core.autocrlf=true`, all nine reported `i/lf w/lf`; `verify-maeos.ps1`, isolated installer, and functional routing passed. Source hashes again match the existing installed files and manifest; no installed file or manifest changed.
